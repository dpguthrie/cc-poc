{% macro create_initial_data(database=None, schema=None) %}
...
{% macro create_update_data(database=None, schema=None) %}
    ...
    {% if execute %}
        ...
        -- Delete those rows now
        delete from {{ database }}.{{ schema }}.dbt_poc_raw_users
        where raw:ID <= 25;

        -- Add in the "updated" rows
        insert into {{ database }}.{{ schema }}.dbt_poc_raw_users(load_id, raw)
        with faker_data as (
            select
                row_number() over (order by null) as id,
                fake('en_US','email', {})::varchar as email,
